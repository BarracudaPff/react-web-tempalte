...
const InputVerificationCode = ({
    ...
    const emptyValue = new Array<string | ReactNode>(length).fill(placeholder)

    const [activeIndex, setActiveIndex] = React.useState<number>(-1)
    const [value, setValue] = React.useState<(string | ReactNode)[]>(
    ...
    const renderItemText = (itemValue: string | ReactNode) => {
    ...
    return (
        <React.Fragment>
            ...
            <S.Container
                className={"input-ver-code__container " + className}
                // needed for styling
                itemsCount={length}
            >
                <S.Input
                    ref={codeInputRef}
                    className="input-ver-code__input"
                ...
                {itemsRef.map((ref, i) => (
                    <S.Item
                        ...
                        role="button"
                        tabIndex={0}
                        className={`input-ver-code__item ${
